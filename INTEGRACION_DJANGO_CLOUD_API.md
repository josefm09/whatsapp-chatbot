# Integración en Django con WhatsApp Business Cloud API (Railway)
...
urlpatterns = [
    path("meta/webhook", views.meta_webhook),
    path("meta/webhook/", views.meta_webhook),
    path("meta/verify", views.meta_verify),
    path("meta/verify/", views.meta_verify),
]
```
...
- Declara zona horaria y `USE_TZ=True` si corresponde.
- Usa variables de entorno en Railway Variables.

## Migraciones y arranque

```bash
python manage.py makemigrations whatsappbot
python manage.py migrate
# Railway: usa tu start command habitual (gunicorn o uvicorn-gunicorn) del ERP
```

## Railway
- No necesitas nuevo servicio si lo integras en el mismo proyecto de Django.
- Agrega variables en el mismo servicio (`WHATSAPP_TOKEN`, `PHONE_NUMBER_ID`, `VERIFY_TOKEN`, etc.).
- En Meta:
  - Verificación (GET): https://<tu-dominio-railway>/meta/verify usando `VERIFY_TOKEN`.
  - Callback (POST): https://<tu-dominio-railway>/meta/webhook y suscribe “messages”.
...
- Usa plantillas aprobadas de WhatsApp para iniciar conversaciones (marketing/autenticación) y optimizar costos.
